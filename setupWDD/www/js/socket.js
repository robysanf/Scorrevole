...
document.addEventListener('deviceready', function () {
	...
	//*********************** EVENTO NUOVA REGISTRAZIONE
	document.getElementById("btn_new").addEventListener("click", activateRegister, false);
	...
	document.getElementById("btn_login").addEventListener("click", function () {
		...
		if(find_db(u,p)){
			...
			document.getElementById("userid").innerHTML=u;
			}else{
			if(user=="unregistred")
			s="Non ci sono utenti registrati. Procedere alla prima registraizone";
			else
			s="User o Password non corrette!!";
			document.getElementById("txt_com").innerHTML=s;
			
		}
		
	});//eventlistner btn_login
	
	//***************************** SALVA NUOVA REGISTRAZIONE  ***********************************
	document.getElementById("btn_register").addEventListener("click", function () {
		
		m=document.getElementById("email").value;
		u=document.getElementById("username").value;
		psw=document.getElementById("psw_reg").value;
		p_r=document.getElementById("psw_reg_repeat").value;
		if(m.trim()=="" || u.trim()=="" || psw.trim()=="" || p_r.trim()=="" )
		{	
			document.getElementById("modalmsg").innerHTML="Valorizzare tutti i campi!";
			document.getElementById("myModal").style.display = "block";
			return;
		}
		if(psw!=p_r){
			document.getElementById("txt_com").innerHTML="Ripetizione password errata!";
			return;
		}
		if(insert_db(u)==2){
			document.getElementById("txt_com").innerHTML="Utente già registrato!";
			document.getElementById("modalmsg").innerHTML="Utente già registrato!";
			document.getElementById("myModal").style.display = "block";
			return;
		}
		else{
			user=u;
			document.getElementById("txt_com").innerHTML="Utente Registrato!!";
			
		}
		
	});//eventlistner btn_register
	
	//**********  INIT   *****************************************
	document.getElementById("appwdd").style.visibility='visible';
    ...
    ws.onopen = function () {
        ...
        this.send('VARS');         // transmit "init" after connecting
	};
    ...
    ws.onmessage = function (event) {
		...
		if(conf.trim()==1 || conf.trim()=="false" )
		{ document.getElementById("message").innerHTML="Configurazione ricevuta dal dispositivo e operativa";
			...
			document.getElementById("message").innerHTML="Necessario eseguire la Configurazione iniziale della porta";
...
function makeConfigHtml(rec){
	...
	if(rec.indexOf("#")==-1){
		...
		createSaveButtonGrid();
		
		ArrayVar=rec.split("#");//ho le stinghe "nome_variabile"=valore
		for(i=0;i<ArrayVar.length;i++){
			
			myArray=ArrayVar[i].split("=");//ho le coppie nome_varibile e valore
			nome_var=""+myArray[0];
			...
			obj_input.id=""+i;
			obj_input.setAttribute("style", "grid-column:1/2 span");
			...
			*/
			...
			x.setAttribute("value", valore_var.trim());
...
function makeResponseToSave() {//costruisce la stringa nome_variabile1=valore1#nome_variabile2=valore2#nome_variabile3=valore3
...
function createSaveButtonGrid(){
	obj_input = document.createElement("div");
	obj_input.className ="grid-item";
	obj_input.id="bsave";
	obj_input.setAttribute("style", "grid-column:1/2 span");
	obj_input.innerHTML="INSERIRE I VALORI DESIDERATI E PREMERE SALVA";
	var x = document.createElement("Button");
	x.setAttribute("onclick", "save(0)");
	x.textContent = 'Save';
